...
def save_linkedin_user_to_db(raw_user:dict, db: Session = Depends(get_db)):
   try:
      ...
      logging.warning('@save_linkedin_user_to_db DB user: %s',user_db)
      existing_user_email = str(user_db.email) if user_db else None
      
      if existing_user_email:
         logger.info("@save_linkedin_user_to_db LinkedIn user already exists in DB: %s", existing_user_email)
         return user_db
      ...
      logger.error("Error saving LinkedIn user to DB: %s", str(e))
